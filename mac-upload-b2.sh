...
#
# Uploads a single local file to a Backblaze B2 bucket using the native
# b2_authorize_account -> b2_get_upload_url -> b2_upload_file API flow.
# No Backblaze CLI required — just bash, curl, and jq.
#
# USAGE:
#   backblaze.sh <APP_ID> <APP_KEY> <BUCKET_ID> <LOCAL_FILE_PATH>
#   backblaze.sh -h | --help
#
# ARGUMENTS:
...
#
# INTERACTIVE MODE:
#   Any omitted argument triggers a prompt. LOCAL_FILE_PATH will keep
#   re-prompting until a valid, existing file path is entered.
#
# NOTES:
#   - Avoid spaces in the filename/path where possible.
#   - Easiest to run from the same directory as the target file.
#   - Requires 'jq' to be installed (script exits with an error if missing).
#   - Uses shasum for SHA1 checksum and stat (BSD/GNU aware) for file size.
#
# REQUIREMENTS:
#   bash, curl, jq, shasum, stat
#
# EXIT CODES:
#   0  Success
#   1  Missing dependency, bad file path, or API failure
#
# VERSION: 1.1
# =============================================================================
